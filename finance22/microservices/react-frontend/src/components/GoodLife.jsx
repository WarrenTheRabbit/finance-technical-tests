...
import GoodLifeDonutChart from './GoodLifeDonutChart';
import GoodLifeTable from './GoodLifeTable';
...
const GoodLife = () => {
  ...
  const { data: expenseData, error, isValidating } = useSWR(
    shouldFetch ? `http://localhost:8000/v1/category?user_id=${user.email}&category=Good Life` : null,
    fetcher,
    {
  ...
  const handleRefresh = () => {
    setShouldFetch(true);
    mutate(`http://localhost:8000/v1/category?user_id=${user.email}&category=Good Life`);
  };
      ...
      <Box sx={{ flexGrow: 1, padding: '20px', marginTop: '5px', bgcolor: "#fff", borderRadius: "16px", maxWidth: '100%', marginLeft: 'auto', marginRight: 'auto' }}>
        ...
        <Grid container spacing={2} justifyContent={"center"} alignItems={"center"}>
          <Grid item xs={12} md={6}>
            <GoodLifeDonutChart data={expenseData || []} />
          </Grid>
          <Grid item xs={12} md={6}>
            <GoodLifeTable data={expenseData || []} />
          </Grid>
        </Grid>
      </Box>
